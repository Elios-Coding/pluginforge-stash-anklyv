...
public final class PluginForgeDispatch {
    ...
    private static synchronized boolean allowGlobal() {
        ...
        if (windowCount >= MAX_DISPATCH_PER_WINDOW) {
            if (!windowWarned) {
                Bukkit.getLogger().warning("[Stashed] dispatch rate limit hit ("
    ...
    private static void run(String cmd, boolean silent) {
        ...
        if (depth >= MAX_DEPTH || depth > 0) {
            Bukkit.getLogger().warning("[Stashed] dispatch depth limit hit, dropping: " + trimmed);
            return;
        }
        ...
        try {
            ...
            Bukkit.getLogger().warning("[Stashed] dispatch failed for '" + trimmed + "': " + t.getMessage());
        } finally {
            seen.remove(key);
            int d = DEPTH.get() - 1;
            DEPTH.set(d);
